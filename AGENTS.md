...
- Runtime URL mapping lives in `src/config/environment.ts`: localhost → frontend `http://localhost:5173`, API `http://localhost:8101`; testing → `testing-storybrand-frontend.bakano.ec` / `testing-storybrand-backapp.bakano.ec`; production → `academia.luisapitabejarano.com` / `luisa-pita-bejarano-backapp.vercel.app`.
